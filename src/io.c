    ...
    if (fgets(buffer, MAXLENGTH, stdin) != NULL)
    {
        ...
        if (buffer != 0)
        {
            ...
            else
            {
                ...
                dd = strtol(buffer, &num_end, 10);

                if (dd == 0 || (dd > 0 && dd <= MAX_DAYS_MONTH))
                {
                    mm = strtol(num_end, &num_end, 10);
                    if (mm == 0 || (mm > 0 && mm <= MAX_MONTHS_YEAR))
                    {
                        yy = strtol(num_end, NULL, 10);
...
errco_t pdata (entry_t * entry)
{
    int bytes;
    bytes = printf(" %s\n %s, %s\n %d.%d.%d\n %d.%d.%d\n ", entry->organization, 
                                            entry->type_of_document, entry->document_name, 
                                            entry->accepted.dd, entry->accepted.mm, entry->accepted.yy,
                                            entry->active_since.dd, entry->active_since.mm, entry->active_since.yy);
    if (bytes == 0)
        return EIO;
    else
    {
