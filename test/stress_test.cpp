...
TEST(stress_test_10k, stress_test_10k) {
  ...
  if (test_input_read != NULL) {
    for (size_t i = 0; i < 10000; ++i) {
      EXPECT_EQ(entry_value_input(&test, test_input_read, NO_OUTPUT),
                EXIT_SUCCESS);
    ...
    auto elapsed_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
        end_time - start_time);
    std::cout << "Stress test chrono results: " << elapsed_ns.count()
              << " ns\n";
    std::cout << "Stress test ticks results: " << (finish - start)
              << " ticks\n";
