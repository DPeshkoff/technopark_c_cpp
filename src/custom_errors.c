...
errco_t errputs(errco_t err_co) {
  switch (err_co) {
    ...
    case 203:
      printf("[Error]:ErrorNoSizeGiven (code %d) \n", err_co);
      break;

    case 204:
      printf("[Error]:ErrorHashTableisNull (code %d) \n", err_co);
      break;

    case 205:
      printf("[Error]:ErrorListSortFail (code %d) \n", err_co);
      break;

    case 206:
      printf("[Error]:ErrorHashTableEmpty (code %d) \n", err_co);
      break;

    case 207:
      printf("[Error]:ErrorWrongHashBasis (code %d) \n", err_co);
      break;
